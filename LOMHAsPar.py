# LOCI WITH MULTIPLE HAPLOTYPES (LOMHAs) DETECTION IN THE PARENTAL POPULATIONS OF A HYBRID SPECIES
...
## Hybrid code part
# Get the hybrid LOMHAs file from the current working directory
import glob
for h_file in glob.glob("h0DFMultiHaplotypes"):
    h0DF = pd.read_csv(h_file, sep="\t", header=None)
    h0DF.rename(columns={0: 'SeqNames', 1:'Seq'}, inplace=True)


# Detect if each of LOMHAs in the hybrids is present in parental populations
# Create output files PatDF for LOMHAs in paternal population and MatDF for LOMHAs in maternal population
# and Mat_PatDF for LOMHAs in both maternal and paternal populations
...
for sq in range(0, len(h0DF)):
    # Inspect if the hybrid haplotype is in paternal side
    inspectorPh = pd.DataFrame([h0DF.loc[sq,'Seq']]).isin(list(p0DF.loc[:, 'Seq'])).any().any()
    # Inspect if the hybrid haplotype is in maternal side
    inspectorMh = pd.DataFrame([h0DF.loc[sq,'Seq']]).isin(list(m0DF.loc[:, 'Seq'])).any().any()

    # Save the LOMHAs as maternal and paternal (if any) in the output files
    if inspectorPh==True:
...
# Export the paternal and maternal LOMHAs files of the hybrid species
PatDF.to_csv("./P_h0DFMultiHaplotypes", sep = '\t', index=False, header=None)
MatDF.to_csv("./M_h0DFMultiHaplotypes", sep = '\t', index=False, header=None)
Mat_PatDF.to_csv("./MP_h0DFMultiHaplotypes", sep = '\t', index=False, header=None)
